...
import type { CoachAdvice, CoachContext, CoachDomain, CoachSignal, CoachTone } from '@/features/coach/types/coach.types'
...
function getLowestPillar(pillarScores: PillarScores) {
  return Object.entries(pillarScores).sort((a, b) => a[1] - b[1])[0]
}

function createAdvice(scenario: AdviceScenario): CoachAdvice {
  return {
    ...scenario,
    signal: {
      ...scenario.signal,
...
function resolveDailyAdvice(context: CoachContext): CoachAdvice {
  const [lowestPillar, lowestScore] = getLowestPillar(context.pillarScores)
  const hasRiskGoal = context.dreams.some((dream) => dream.status === 'at_risk')
  const hasStudyRiskMemory = context.memories.some((memory) =>
    memory.title.toLowerCase().includes('hábitos em risco') || memory.description.toLowerCase().includes('leitura'),
  )
  const currentPeriod = context.evolution.periods.find((period) => period.id === 'today')

  if (context.lifeScore < 50 || context.pendingHabits > context.completedHabits + 1) {
    return createAdvice(coachAdviceScenarios[3])
  }

  if (lowestPillar === 'Conhecimento' || hasRiskGoal || hasStudyRiskMemory) {
    return createAdvice(coachAdviceScenarios[0])
  }

  if (lowestPillar === 'Saúde' || (currentPeriod?.workouts ?? 0) === 0) {
    return createAdvice(coachAdviceScenarios[1])
  }

  if (context.spiritualEnabled !== false && lowestPillar === 'Fé') {
    return createAdvice(coachAdviceScenarios[2])
  }

  if (context.lifeScore >= 70 && (currentPeriod?.streak ?? 0) >= 7) {
    return createAdvice(coachAdviceScenarios[4])
  }

  if (lowestScore <= 16) {
    return createAdvice({
      ...coachAdviceScenarios[0],
      message: `O pilar ${lowestPillar} merece uma ação pequena e concreta hoje.`,
      action: `Faça uma ação de 15 minutos ligada a ${lowestPillar}.`,
      reason: `Esse é o menor pilar do seu painel agora.`,
    })
  }
...
export async function getDailyCoachAdvice(input: DailyCoachInput): Promise<CoachAdvice> {
  const dreams = getDreamEngineMock()
  const memories = getMemoryEngineMock()
  const evolution = await getEvolutionHistoryMock()

  return resolveDailyAdvice({
    ...input,
